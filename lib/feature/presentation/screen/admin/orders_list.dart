...
class _OrdersListState extends ConsumerState<OrdersList> {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final orderCartItems = cartItems
                  ...
                  .toList();
              final patientsName =
                  patients.firstWhere((e) => e.id == order.userId).name;
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          if (order.status == 'pending')
                            _button(Icons.approval, order),
                          if (order.status == 'approved')
                            _button(Icons.check_circle, order),
                        ],
                      ),
  ...
  InkWell _button(IconData icon, OrdersModel order) {
    return InkWell(
      onTap: () {
        if (icon == Icons.approval) {
          ref.read(ordersProvider.notifier).changeStatus(
                model: OrdersModel(
                  id: order.id,
                  statusOption: 0,
                ),
